...
def getCurrentManifest():
    ...
    response = urlopen(f"https://raw.githubusercontent.com/{repo_owner}/{repo}/master/manifest.json")
...
def githubChanges(file_name: str):
    req_tag = Request(f"https://api.github.com/repos/{repo_owner}/{repo}/tags")
    req_release = Request(f"https://api.github.com/repos/{repo_owner}/{repo}/releases/latest")
    ...
    req_commits = Request(f"https://api.github.com/repos/{repo_owner}/{repo}/commits", headers=header)
...
def modsChanges(new_manifest_data: dict, file_name: str):
    ...
    with open(file_name, "a", encoding="utf-8") as f:
        appendFile(f, f"# Changelog for {modpack_name} v" + manifest_data['version'])
